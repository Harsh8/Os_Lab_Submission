...
int bestFit(int bsize){
	...
	else if(bsize>300 && bsize<=500){
		j=3;
	}
	else{
		printf("\nEnter a valid size of memory block(100, 200, 300, 500): ");
		return 0;
	}
	while(j!=4){
...
void release(){
	...
	do{
		...
		scanf("%d",&rel);
		if(rel){
			printf("\nEnter the block size(KBs) which you want to release(100, 200, 300, 500): ");
			scanf("%d",&memSize);
			switch(memSize){
				case 100:
					indexFree[0][1]=1;
				...
				case 500:
					...
					break;
				default:
					printf("\nEnter valid block size (100, 200, 300, 500): ");
			}
		}
	}while(rel);
	return;
